...
public class OrchestratorService {
    ...
    public void setOrchestrator(String ip, String port) {
        ...
        orchestrator = new Node(ip, port, md5HashStr, true);
    }

    public void callOrchestratorToRegister(String ip, String port) {
        String url = String.format("http://%s:%s%s?ip=%s&port=%s", orchestrator.getIp(), orchestrator.getPort(), KVUrl.DO_REGISTER, ip, port);
